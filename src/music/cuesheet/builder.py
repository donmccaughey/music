...
class Builder:
    ...
    def build_cuesheet(self) -> CueSheet2:
        cuesheet = CueSheet2()
        for child in self.root.children:
            match child:
                ...
                case File() as file_node:
                    cuesheet.file = self.build_cuesheet_file(file_node)
    ...
    def build_cuesheet_file(self, file_node: File) -> File2:
        ...
        for child in file_node.children:
            match child:
                ...
                case Track() as track_node:
                    track = self.build_cuesheet_file_track(track_node)
                    file.tracks.append(track)
                case _:
                    raise RuntimeError(f'Unexpected node {child}')
        return file

    def build_cuesheet_file_track(self, track_node: Track) -> Track2:
